...
export function createLead(name: string, email: string, source: string = 'Chatbot') {
  const id = randomUUID();
...
export function updateLeadScore(email: string, score: string) {
  const stmt = crmDb.prepare(`
...
export function getLeadsByScore(score: string) {
  const stmt = crmDb.prepare('SELECT * FROM leads WHERE lead_score = ? ORDER BY created_at DESC');
  return stmt.all(score);
}

export function createInteraction(leadEmail: string, type: string, content: string, aiSummary?: string) {
  const stmt = crmDb.prepare(`
...
export function getInteractionsByLead(email: string) {
  const stmt = crmDb.prepare(`
